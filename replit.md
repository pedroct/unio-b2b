...
- `GET /api/longevidade/clientes/:id/performance-funcional?intervalo=7d|30d|90d` → `RespostaPerformanceFuncional` { cliente_id, score: ScoreHeader | null, biomarcadores: { exercise_minutes, walking_speed, stability, strength, heart_rate_zones: HeartRateZones | null, historico_exercicios: HistoricoExercicios | null } }. HeartRateZones: zone1-4_minutes em minutos. `historico_exercicios` inclui `intervalo`, `total_sessoes`, `resumo_por_tipo: Record<string, ResumoPorTipoItem>`, `sessoes: SessaoExercicio[]`. Proxy repassa `?intervalo` para staging.
