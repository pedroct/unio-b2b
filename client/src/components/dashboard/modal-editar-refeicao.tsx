...
export function ModalEditarRefeicao({
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormData) => {
      const alimentosPayload = data.alimentos.map((a) => ({
      ...
      await apiRequest(
        "PUT",
        `/api/profissional/dashboard/pacientes/${pacienteId}/planos-alimentares/${planoId}/refeicoes/${refeicao.id}`,
        {
          nome: data.descricao.trim(),
